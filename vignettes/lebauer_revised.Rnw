...
\section{Translating R  parameterizations to BUGS (and back again)}
 ...
 Finally, the spelling of some function names is changed (Table~\ref{tab:naming}).
  ...
  The \strong{Normal} and \strong{log-normal} distributions use mean ($\mu$) as the first argument in both languages, but the second argument is standard deviation ($\sigma$) in R and precision ($\tau=1/\sigma^2$) in BUGS.
